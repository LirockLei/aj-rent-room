...
$(document).ready(function(){
        ...
        success: function(data){
            if(data.code == '200'){
                ...
                for(index in data.data){
                    ...
                    var liNode = $('<li area-id="' + area.area_id + '">' + area.area_name + '</li>')
    ...
    $.get('/home/search/', function(data){
        var area_id = queryData["aid"]
        console.log(area_id)
        data = {'area_id': area_id}
        $.get('/home/my_search/', data, function(data){
                if(data.code == '200'){
                    console.log(data)
                    for(index in data.data){
                        ...
                        var imgNode = $('<img/>').attr('src', house.images[0])

                        var div1Node = $('<div class="house-desc"></div>')

                        var div11Node = $('<div class="landlord-pic"></div>')

                        var img2Node = $('<img/>').attr('src', '/static/media/' + house.user_avatar + '/')

                        var div12Node = $('<div class="house-price">￥<span>'+ house.price + '</span>/晚</div>')

                        var div13Node = $('<div class="house-intro"></div>')

                        var spanNode = $('<span class="house-title">' + house.title + '</span>')

                        var emNode = $('<em>出租' + house.room_count + '间 - ' + house.order_count + '次入住 - ' + house.address + '</em>')
                        ...
                        div1Node.append(div13Node)
                        div13Node.append(spanNode)
                        div13Node.append(emNode)
                    }
                }
        })
    })
